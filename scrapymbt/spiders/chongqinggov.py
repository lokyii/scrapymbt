...
class ChongqinggovSpider(scrapy.Spider):
    name = 'chongqinggov'
    ...
    def __init__(self, timeout=None, start_date=None, end_date=None, province_policy_kw=None):
        # 继承父类的init方法
        super(ChongqinggovSpider, self).__init__()
    ...
    def parse(self, response):
        ...
        if rows is not None:
            for row in rows:
                created_date = row.xpath('./div[@class="description"]/div/ul/li[3]/span[3]/text()').extract_first().strip()
    ...
    def item_parse(self, response):
        ...
        item['content'] = ''.join(i.strip().replace('\r', '').replace('\u3000', '').replace(u'\xa0', '').replace(u'\u2002', '')
                                  for i in response.xpath('//div[starts-with(@class,"view TRS_UEDITOR trs_paper_default")]//p//text()').extract())
